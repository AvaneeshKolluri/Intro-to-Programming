...
def findRunBits(s):
    ...
    s = listOfRunLengths(s)
    return min(math.ceil(math.log(sum(s)/len(s))) + 1,7)
...
def test():
    """Test functions foir Uncompress and Compress funcions."""
    assert uncompressA(compressA(L0)) == '1110001111001010'
    assert uncompressA(compressA(L1)) == '1'
    assert uncompressA(compressA(L2)) == '111111110101010101010101010101010101010101'
    assert uncompressA(compressA(L3)) == '110011001100'
    assert uncompressA(compressA(L4)) == '000111000111'
    assert uncompressA(compressA(L5)) == '0000111100001111'
    assert uncompressA(compressA(L6)) == '0' * MAX_RUN_LENGTH + '1' * MAX_RUN_LENGTH + '0' * (64 - 2 * MAX_RUN_LENGTH)
    assert uncompressA(compressA(L7)) == '0' * (MAX_RUN_LENGTH + 1) + '1' * (MAX_RUN_LENGTH + 1) + '0' * (64 - 2 * MAX_RUN_LENGTH - 2)
    assert uncompressA(compressA(L8)) == '1' * MAX_RUN_LENGTH + '0' * MAX_RUN_LENGTH + '1' * (64 - 2 * MAX_RUN_LENGTH)

    assert compressA(L0) == '0100011111100011001010101'
    assert compressA(L1) == '00101'
    assert compressA(L2) == '01000110011001001010101010101010101010101010101010101010101010101010101010101010101'
    assert compressA(L3) == '01000101010101010'
    assert compressA(L4) == '011011011011011'
    assert compressA(L5) == '011100100100100'
    assert compressA(L6) == '101111111111100010'
    assert compressA(L7) == '101111110000000001111110000000001'
    assert compressA(L8) == '10100000111111111100010'
